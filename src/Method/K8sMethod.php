...
class K8sMethod extends BaseMethod implements MethodInterface
{
    ...
    public function preflightTask(string $task, HostConfig $config, TaskContextInterface $context)
    {
        ...
        if (empty($config['kube']['podForCli'])) {
            $kube_config = $config['kube'];
            $kube_config['podForCli'] = $this->getPodNameFromSelector($config, $context);
            $config['kube'] = $kube_config;
        }
    }
        ...
        try {
            $result = $this->kubectl(
                ...
                sprintf(
                    ...
                    implode(',', $host_config['kube']['podSelector'])
                ),
                true
            );
            ...
            $this->logger->warning("Could not get name of pod from selectors ". $e->getMessage());
        }
