...
{
    public const CONFIGURATION_EXISTS = 'configurationExists';
    public const CONFIGURATION_USED = 'configurationUsed';
    public const SKIP_CONFIGURATION_IMPORT = 'skipConfigurationImport';
    public const SKIP_NEXT_CONFIGURATION_IMPORT = 'skipNextConfigurationImport';
    public const SETTINGS_FILE_EXISTS = 'settingsFileExists';

    public const LAX_ERROR_HANDLING = 'lax';
    public const STRICT_ERROR_HANDLING = 'strict';
    ...
    private function useStrictErrorHandling(HostConfig $host_config): bool
    {
        ...
        return self::STRICT_ERROR_HANDLING == $drush_error_handling;
    }
    ...
    public function variables(HostConfig $host_config, TaskContextInterface $context)
    {
        ...
        foreach ($result as $key => $value) {
            ...
            } elseif ('push' == $what && !is_null($value)) {
    ...
    private function getVariable(HostConfig $host_config, TaskContextInterface $context, $key)
    {
        ...
        if (7 == $host_config['drupalVersion']) {
    ...
    public function requestDatabaseCredentialsAndWorkingDir(HostConfig $host_config, TaskContextInterface $context)
    {
        ...
        if (empty($data)) {
            ...
            if (json_last_error()) {
                throw new \RuntimeException(sprintf("Could not parse drush's database credentials: `%s`:\n%s", json_last_error_msg(), $payload));
            }
            $defaults = [
                'mysql' => [
                    'host' => 'localhost',
