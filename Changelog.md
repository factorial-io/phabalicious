...
### Changed
  ...
  * Refactored and renamed method `ftp-sync` to `artifacts--ftp-sync` in preparation of git-sync.
 
### New
  * `rootFolder` is set by default now
  * All context variables are exposed as replacement patterns for using in scripts.
  * new method `artifacts--git-sync` to build an artifact and push it to a git repository, see new documentation about artifacts.
